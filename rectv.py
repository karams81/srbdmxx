...
# --- BÖLÜM 3: M3U OLUŞTURMA (ORİJİNAL FORMAT) ---
...
def generate_m3u():
    ...
    with open(OUTPUT_FILENAME, "w", encoding="utf-8") as f:
        ...
        for cat_id, cat_name in movie_categories:
            movie_base_url = f"{MAIN_URL}/api/movie/by/filtres/{cat_id}/created/"
            movies = get_all_pages(movie_base_url, cat_name)
            for movie in movies:
        ...
        for cat_id, cat_name in series_categories:
            serie_base_url = f"{MAIN_URL}/api/serie/by/filtres/{cat_id}/created/"
            all_series.extend(get_all_pages(serie_base_url, cat_name))
        
        unique_series = list({s['id']: s for s in all_series}.values())
        print(f"\nToplam {len(unique_series)} benzersiz dizi için bölümler taranıyor...", file=sys.stderr)
        
        for serie in tqdm(unique_series, desc="Diziler İşleniyor"):
